...
def run_full_update(task):
    ...
    try:
        ...
        prompt = f"""
        你是一位專業財務資料整理人員。請根據 {symbol} 的財務數據進行中性描述，並回傳純 JSON。
        ...
        - 不得提供投資建議、操作建議或買賣建議
        - 不得使用推薦、判斷優劣或評價性語句
        - 避免使用如「強勁、疲弱、健康、危險、便宜、高估、低估、優秀、惡化」等帶有主觀判斷的形容詞
        - 可使用「呈現上升趨勢」「位於較高區間」「變動幅度有限」等客觀描述
        - 若資料不足或變動極小，請直接描述數據特性

        格式要求：
        - "one_liner": 財務狀況一句話總結（僅可為客觀特徵描述）
        - "by_metric": {{
            "每個指定的 ID": {{
                "summary": "數據解讀摘要（需為中性描述）",
                "bullets": ["趨勢觀察1", "趨勢觀察2", "趨勢觀察3"],
                "related_reasons": ["相關背景因素1", "相關背景因素2", "相關背景因素3"]
            }}
        }}
        - "risks": ["由數據可觀察到的不確定性或波動來源"]

        分析要求：必須使用繁體中文。
        即便數據為 0 或變動極小，也請針對該指標提供簡短客觀描述。
