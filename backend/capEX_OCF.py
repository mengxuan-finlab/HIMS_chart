...
# 移除缺值列
df = df.dropna(subset=["capitalExpenditure"])
...
records = [
    {
        "date": str(d),
        "capEX_OCF": float(cE/ocf)
    }
    for d, ocf , cE, in zip(df["date"], df["operatingCashFlow"],df["capitalExpenditure"])
]

# 寫出到 JSON 檔案
out_path = Path(__file__).parent / "hims_capEX_OCF.json"
