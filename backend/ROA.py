...
ROE_ttm = data2[0]["returnOnAssetsTTM"]

# 移除缺值列
ROA = ROE.dropna(subset=["returnOnAssets"])
...
records = [
    ...
    for d, ROA in zip(ROA["date"], ROA["returnOnAssets"])
]
...
if ROE_ttm is not None:
    records.append({
        "date": "TTM",
        "returnOnAssets": round(float(ROE_ttm), 2)
    })
# 寫出到 JSON 檔案
out_path = Path(__file__).parent / "hims_ROA.json"
